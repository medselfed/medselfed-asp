...
%>
  ...
  <tr <% if colorOn = "true" then %> bgcolor="#CCCCCC" <% colorOn = "false"%><% else %><% colorOn = "true"%><% end if %> >
	...
	<option value="0" <% if Rs.Fields("questions") = "0" then %>selected<% end if %>>No</option>
       ...
       <td>
	    <input class="altButtonFormat" type="submit" name="action" value="update"/>
      ...
      <td class="sml"><a href="editPartnerBar.asp?partnerId=<% = Rs.Fields("partnerId") %>">navigation</a><br> 
		...
		<a href="editDiscliamer.asp?partnerId=<% = Rs.Fields("partnerId") %>">discliamer</a><br>
        <a href="editSpecailInstr.asp?partnerId=<% = Rs.Fields("partnerId") %>">special instructions</a><br>
        <a href="generatePartnerMRSS.asp?partnerId=<% = Rs.Fields("partnerId") %>">Generate 
        MRSS Feed</a><br>
        <a href="../../xml/partners/<% = Rs.Fields("partnerId") %>_mrss.xml" target="_blank">Go To 
        MRSS Feed</a><br>
		<a href="generatePartner.asp?partnerId=<% = Rs.Fields("partnerId") %>">Generate 
        XML</a><br>
		<a href="http://<% = Rs.Fields("partnerId") %>.medselfed.com/asp/home.asp" target="_blank">Go To Site</a><br>
		<a href="../../xml/partners/<% = Rs.Fields("partnerId") %>.xml" target="_blank">Go To XML</a><br></td>
    </tr>
  </form>
  <% Rs.MoveNext %>
  <% Loop %>
  ...
  <form name="partnerUpdate" action="editDatabase.asp">
	...
	<input type="hidden" name="discliamer" value="Insert Discliamer">
    <input type="hidden" name="show_google" value="0">
    <tr bgcolor="#CCCCCC"> 
      <td class="smlred">
	  Partner ID:<br>
